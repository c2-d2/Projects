...
ggsave("figure1.pdf", figure1, width = 14,height = 7)
```

***

## Figure 1 – Suppl 1: Tree
...
plotTree<-function(tree,ladderise=NULL,heatmapData=NULL,barData=NULL,infoFile=NULL,
  ...
  if (!is.null(heatmapData)) {
    ...
    image((1:ncol(y.ordered))-0.5,(1:nrow(y.ordered))-0.5, as.matrix(t(y.ordered)),col=heatmap.colours,breaks=heatmapBreaks,
  ...
  if (!is.null(barData)) {
    par(mar=rep(0,4))
    barplot(barData[tip.label.order], horiz=T, axes=F, xaxs="i", yaxs="i", xlab="", ylab="", ylim=c(0.25,length(barData)+0.25),xlim=c((-1)*max(barData,na.rm=T)/20,max(barData,na.rm=T)),
...
ggsave("figure2.pdf", figure2, width = 14, height = 4.5)
```
...
plot3A <- ggplot(dat3AB, aes(x=deltaE, y=r, colour = vaccine)) + 
  ...
  geom_point(size = 3, alpha = 0.9) + theme(legend.position = "none", 
                               axis.text = element_text(colour = "black"),) + 
...
plot3B <- ggplot(SC_freq_postV_obs, 
  ...
  annotate(geom = "text", x=0.112, y =0.116, 
...
plot3C <- ggplot(SC_freq_postV_diff, aes(x = diff_pred, 
  ...
  annotate(geom = "text", x=0.09, y=0.095, 
           label = "1:1 line", angle = 45, size = 3) + 
  geom_point(size=3, alpha = 0.9) + theme_classic() +
  scale_colour_manual(values = c("#143c77","mediumorchid4"),
  ...
  annotate("text", x=-0.049, y=0.09, size=2.5,hjust = 0,
...
ggsave("figure3.pdf", plot3, width = 16, height = 4)
ggsave("figure3.png", plot3, width = 16, height = 4)
```

***

## Figure 3 - Suppl 1: Prediction Mass US Data
...
figure3SA <- ggplot(datPlotAMA, aes(x=SC, y=Prevalence, alpha=Epoch, fill=vaccine)) +
  geom_bar(stat='identity', position='dodge') + 
  scale_alpha_manual(values = c(1,0.4),
                     labels = c("Pre-vaccine", "Post-vaccine")) +
  scale_fill_manual(values = c("NVT"="#143c77","VT"="darkred",
                               "VT_NVT"="mediumpurple4"),
                    labels = c("NVT"="Non-vaccine type", 
                               "VT"="Vaccine type", "VT_NVT"="Mixed"), 
                    name = "Composition") +
  xlab("Strain (SC)") + theme_classic() + 
...
#### Plot B: Change in prevalence ####
figure3SB <- ggplot() + ylim(c(-0.2,0.35)) +
  geom_hline(yintercept=0, lty="dashed",size=0.65) +
  scale_colour_manual(values = c("NVT"="#143c77","VT"="darkred",
                                 "VT_NVT"="mediumpurple4"),
                      labels = c("NVT"="Non-vaccine type", 
                                 "VT"="Vaccine type", "VT_NVT"="Mixed"), 
                      name = "SC Composition") +
...
#### Figure C ####
plot3SC <- ggplot(SC_freq_postV_obsMA, 
                 aes(x = SC_pred, y = SC_freq, colour = vaccine)) +
  theme_classic() + 
  ...
  geom_smooth(method='lm',color="#899DA4",
              formula=y~x, alpha=0.3, lwd=.6, 
              fullrange=T, linetype="blank", show.legend=F) + 
  annotate(geom = "text", x=0.275, y =0.29, 
           label = "1:1 line", angle = 45, size = 3) + 
  geom_point(size = 3, alpha = 0.9) + 
...
ars <- round(stats$adj.r.squared, digits = 3)
sseE <- round(sse(SC_freq_postV_diffMA$diff_obs, 
                  SC_freq_postV_diffMA$diff_pred), digits = 3)
maeE <- round(mae(SC_freq_postV_diffMA$diff_obs, 
                  SC_freq_postV_diffMA$diff_pred),digits = 3)##Mean Absolute Error
rmseE <- round(rmse(SC_freq_postV_diffMA$diff_obs, 
                    SC_freq_postV_diffMA$diff_pred),digits = 3) ##Root Mean Squared Error
...
#### Figure D ####
plot3SD <- ggplot(SC_freq_postV_diffMA, 
  ...
  geom_smooth(method='lm', color="gray80" ,formula=y~x, 
              alpha=0.3, lwd=.6, fullrange=T, 
              linetype="blank", show.legend=F) +
  annotate(geom = "text", x=0.18, y =0.19, 
           label = "1:1 line", angle = 45, size = 3) + 
  geom_point(size = 3, alpha = 0.9) + theme_classic() +
  scale_colour_manual(values = c("#143c77","mediumorchid4"),
                      labels = c("Non-vaccine type", "Mixed"), 
  ...
  annotate("text",x=-0.05,y=0.19, size=2.5,hjust = 0,
           label=paste("SSE = ", sseE, "\nRMSE = ", 
                       rmseE, "\nAdj. R2 = ", ars)) 
...
ars <- round(stats$adj.r.squared, digits = 3)
sseE <- round(sse(SC_freq_postV_diffMA$diff_obs, 
                  SC_freq_postV_diffMA$diff_predPro), digits = 3)
maeE <- round(mae(SC_freq_postV_diffMA$diff_obs, 
                  SC_freq_postV_diffMA$diff_predPro),digits = 3)##Mean Absolute Error
rmseE <- round(rmse(SC_freq_postV_diffMA$diff_obs, 
                    SC_freq_postV_diffMA$diff_predPro),digits = 3) ##Root Mean Squared Error
...
plot3SE <- ggplot(SC_freq_postV_diffMA, 
                  aes(x = diff_predPro, y = diff_obs, colour = vaccine)) + 
  geom_segment(aes(x=-0.05,xend=0.2,y=-0.05,yend=0.2),
               color="black",alpha=.7,lwd=0.5,lty=3) +
  geom_smooth(method='lm', color="gray80" ,formula=y~x, 
              alpha=0.3, lwd=.6, fullrange=T, 
...
figure3SAB <- plot_grid(figure3SA, figure3SB, labels = c("A","B"), ncol = 2)
figure3SCE <- plot_grid(plot3SC, plot3SD, plot3SE, 
                       nrow = 1, labels=c("C","D","E"))

plot3S1 <- plot_grid(figure3SAB,figure3SCE, ncol=1)
ggsave("figure3S1.pdf", plot3S1, width = 12, height = 6)
ggsave("figure3S1.png", plot3S1, width = 12, height = 6)
```

***

## Figure 4: Core versus Accessory Genome  distances
```{r echo=TRUE}
...
plot4A <- ggplot(MergedDistancesMedian, aes(PCore,PAcc)) +
  geom_point(color="black",size=1) + theme_classic() + 
...
rownames(RF.E1.distmat) <- RelativeFitness.E1$BAPS2
colnames(RF.E1.distmat) <- RelativeFitness.E1$BAPS2
...
plot4B <- ggplot(CorDist_RelFit, aes(PCore,RelFit)) +
  geom_point(color="black",size=1) + theme_classic() + 
  geom_density_2d(color="#08519c", alpha=.7, show.legend =FALSE) + 
  labs(x="Core Genome Divergence (Patristic Distance)") + 
...
plot4C <- ggplot(CorDist_RelFit, aes(PAcc,RelFit)) +
  geom_point(color="black",size=1) + theme_classic() + 
  geom_density_2d(color="#08519c", alpha=.7, show.legend =FALSE) + 
...
plot4 <- plot_grid(ncol=3, nrow=1, plot4A, plot4B, 
                      plot4C, labels = "AUTO", align = 'h')
ggsave("figure4.png", plot4, width = 12, height = 4)
ggsave("figure4.pdf", plot4, width = 12, height = 4)

```

***

## Figure 5: Distribution of COGs by SCs
...
plot5 <- ggplot(SC_freq_E13, aes(f, fill = Epoch)) + 
...
ggsave("figure5.png", plot5, width = 15, height = 5)
ggsave("figure5.pdf", plot5, width = 15, height = 5)
```
...
write.csv(dfOut, "table1_stats.csv", row.names = F)
```
