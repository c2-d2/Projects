...
#### Figure C ####
plotSC <- ggplot(SC_freq_postV_obsMA, 
                 aes(x = SC_pred, y = SC_freq, colour = vaccine)) +
  theme_classic() + 
...
#### Figure D ####
plotSD <- ggplot(SC_freq_postV_diffMA, 
...
plotSE <- ggplot(SC_freq_postV_diffMA, aes(x = diff_predPro, 
                                           y = diff_obs, colour = vaccine)) + 
  geom_segment(aes(x=-0.2,xend=0.025,y=-0.2,yend=0.025),
               color="black",alpha=.7,lwd=0.5,lty=3) +
  geom_smooth(method='lm', color="gray80" ,formula=y~x, 
...
figureSCE <- plot_grid(plotSC, plotSD, plotSE, 
                       nrow = 1, labels=c("C","D","E"))

plotSPARC <- plot_grid(figureSAB,figureSCE, ncol=1)
